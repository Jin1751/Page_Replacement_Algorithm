...
This code implements one of Page Replacement Algorithm in OS (FIFO)
*/
...
int main()
{
    int r_len, reference[50], f_len, frame[20], index = 0, pf = 0, max_index = 0, is_same = 0, hit = 0;
    int f0[50], f1[50], f2[50], h[50];// array f0, f1, f2 is to record each index of frame array, Need more arrays if frame is bigger than 3
    int *arr, isHit = 0;
    ...
    for (int i = 0; i < r_len; i++)
    {
        ...
        {
            if (index < f_len)//If frame array has empty index
            {
                frame[index] = reference[i];//Put process to frame array 
                index++;
            ...
            {
                for (int k = 0; k < f_len - 1; k++)
                {
                    frame[k] = frame[k + 1];
                }//Move processes 1 step forward (Delete process in first index) 
                frame[f_len - 1] = reference[i];//Put current process to last index of frame array
                pf++;//Increse 1 Page Fault
                is_same = 1;
                f0[i] = frame[0];//array 'f0' is to print which processes were in frame 0, Initialize f0[i]  to frame[0] 
                f1[i] = frame[1];//array 'f1' is to print which processes were in frame 1, Initialize f1[i]  to frame[1] 
                f2[i] = frame[2];//array 'f2' is to print which processes were in frame 2, Initialize f2[i]  to frame[2] 
